...
def health_check():
    ...
    middleware_ok = all(
        mw in settings.MIDDLEWARE for mw in required_middleware)
    if middleware_ok:
    ...
    print("\n" + "=" * 50)
    print(
        f"Health Check Results: {checks_passed}/{total_checks} checks passed")
...
def database_info():
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT version()" if 'postgresql' in db_config.get(
                'ENGINE', '') else "SELECT sqlite_version()")
